...
def build_friction_collision_appearance_profile(
    *,
    ...
    if not frame_paths:
        raise ValueError("friction-collision appearance requires source frames")

    canonical_by_object_id = shared._canonical_object_ids(object_plan)
    role_by_canonical_id = shared._role_by_canonical_id(
        object_plan,
    ...
    for canonical_id, object_ids in grouped_objects.items():
        ...
        if not len(samples):
            raise ValueError(
                f"no friction-collision appearance samples for {canonical_id}"
            )
        ...
        object_profiles[canonical_id] = {
            ...
            "render_albedo_scale": 1.0,
            **shared._color_statistics(samples),
        }
    ...
    representative_frames = shared._uniform_indices(
        int(seg1[0]), int(seg1[1]), 6
    )
    if len(seg2) == 2:
    ...
    for frame_index in representative_frames:
        ...
        bounds = {
            "back_wall": (0, int(0.28 * height), 0, width),
            "floor": (int(0.50 * height), height, 0, width),
        }
    ...
    environment["floor"].update(
        {
            "render_albedo_scale": 0.68,
            "texture_scale": 640.0,
            "texture_dark_scale": 0.48,
            "texture_light_scale": 1.04,
        }
    )
    ...
    curtain_sample_start = int(round(core_start + 0.25 * (core_end - core_start)))
    curtain_sample_end = int(round(core_end - 0.25 * (core_end - core_start)))
    curtain_samples = []
    for frame_index in shared._uniform_indices(
        curtain_sample_start,
        curtain_sample_end,
        3,
    ):
        image = load_frame(frame_index)
    ...
    return {
        ...
        "scenario": scenario,
        "method": (
            "shared refined two-segment SAM3-mask sampling with "
            "friction-collision room regions"
        ),
