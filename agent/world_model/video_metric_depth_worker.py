...
DEFAULT_VIDEO_METRIC_DEPTH_WORKER_CMD = (
    "python scripts/world_model/video_metric_depth_worker.py"
)
...
class VideoMetricDepthWorkerClient:
    ...
    def start(self) -> Dict[str, Any]:
        ...
        command = os.getenv("PHYSMIND_VIDEO_METRIC_DEPTH_WORKER_CMD", DEFAULT_VIDEO_METRIC_DEPTH_WORKER_CMD)
        ...
        while True:
            ...
            if self.process.poll() is not None:
                raise RuntimeError(
                    "Video metric-depth worker exited before ready. "
            ...
            if time.perf_counter() - start > self.startup_timeout_sec:
                ...
                raise TimeoutError(
                    f"Video metric-depth worker did not become ready within {self.startup_timeout_sec:.1f}s"
                )
            time.sleep(0.25)

    def request(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        if self.process is None or self.process.poll() is not None:
            raise RuntimeError("Video metric-depth worker is not running.")
        ...
        while True:
            ...
            if self.process.poll() is not None:
                raise RuntimeError(
                    "Video metric-depth worker exited while processing request. "
                    f"stdout_tail={_tail(self.stdout_path)!r} stderr_tail={_tail(self.stderr_path)!r}"
                )
            if time.perf_counter() - start > self.request_timeout_sec:
                raise TimeoutError(
                    f"Video metric-depth worker request timed out after {self.request_timeout_sec:.1f}s"
                )
            time.sleep(0.1)
