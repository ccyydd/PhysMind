...
DEFAULT_GEOCALIB_WORKER_CMD = "python scripts/world_model/geocalib_worker.py"
...
class GeoCalibWorkerClient:
    def __init__(self, run_dir: Path, *, startup_timeout_sec: float = 900.0, request_timeout_sec: float = 1800.0):
        self.run_dir = run_dir
        self.worker_dir = run_dir / "workers" / "geocalib"
    ...
    def start(self) -> Dict[str, Any]:
        ...
        command = os.getenv("PHYSMIND_GEOCALIB_WORKER_CMD", DEFAULT_GEOCALIB_WORKER_CMD)
        ...
        while True:
            ...
            if self.process.poll() is not None:
                raise RuntimeError(
                    "GeoCalib worker exited before ready. "
            ...
            if time.perf_counter() - start > self.startup_timeout_sec:
                ...
                raise TimeoutError(f"GeoCalib worker did not become ready within {self.startup_timeout_sec:.1f}s")
            time.sleep(0.25)

    def request(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        if self.process is None or self.process.poll() is not None:
            raise RuntimeError("GeoCalib worker is not running.")
        ...
        while True:
            ...
            if self.process.poll() is not None:
                raise RuntimeError(
                    "GeoCalib worker exited while processing request. "
                    f"stdout_tail={_tail(self.stdout_path)!r} stderr_tail={_tail(self.stderr_path)!r}"
                )
            if time.perf_counter() - start > self.request_timeout_sec:
                raise TimeoutError(f"GeoCalib worker request timed out after {self.request_timeout_sec:.1f}s")
            time.sleep(0.1)
