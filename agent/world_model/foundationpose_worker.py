...
DEFAULT_FOUNDATIONPOSE_WORKER_CMD = "python scripts/world_model/foundationpose_worker.py"
...
class FoundationPoseWorkerClient:
    def __init__(self, run_dir: Path, *, startup_timeout_sec: float = 900.0, request_timeout_sec: float = 7200.0):
        self.run_dir = run_dir
        self.worker_dir = run_dir / "workers" / "foundationpose"
    ...
    def start(self) -> Dict[str, Any]:
        ...
        command = os.getenv("PHYSMIND_FOUNDATIONPOSE_WORKER_CMD", DEFAULT_FOUNDATIONPOSE_WORKER_CMD)
        ...
        while True:
            ...
            if self.process.poll() is not None:
                raise RuntimeError(
                    "FoundationPose worker exited before ready. "
            ...
            if time.perf_counter() - start > self.startup_timeout_sec:
                ...
                raise TimeoutError(
                    f"FoundationPose worker did not become ready within {self.startup_timeout_sec:.1f}s"
                )
            time.sleep(0.25)

    def request(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        if self.process is None or self.process.poll() is not None:
            raise RuntimeError("FoundationPose worker is not running.")
        ...
        while True:
            ...
            if self.process.poll() is not None:
                raise RuntimeError(
                    "FoundationPose worker exited while processing request. "
                    f"stdout_tail={_tail(self.stdout_path)!r} stderr_tail={_tail(self.stderr_path)!r}"
                )
            if time.perf_counter() - start > self.request_timeout_sec:
                raise TimeoutError(f"FoundationPose worker request timed out after {self.request_timeout_sec:.1f}s")
            time.sleep(0.1)
